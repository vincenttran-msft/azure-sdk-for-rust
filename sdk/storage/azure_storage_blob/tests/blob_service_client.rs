...
async fn test_find_blobs_by_tags_service(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    create_test_blob(
        ...
        Some(BlockBlobClientUploadOptions::default().with_tags([("foo", "bar")])),
    )
    .await?;
    ...
    create_test_blob(
        ...
        Some(BlockBlobClientUploadOptions::default().with_tags([("fizz", "buzz")])),
    )
    .await?;
    ...
    create_test_blob(
        ...
        Some(BlockBlobClientUploadOptions::default().with_tags(&blob3_tags)),
    )
    .await?;
